...
const ServiceCard = ({service,index}) => {
    ...
    // sets mouse position inside the card
    const[visible,setVisible]=useState(false);

    const divRef=useRef(null);
    // gives direct access to DOM

    const handleMouseMove=(e)=>{
        const bounds=divRef.current.getBoundingClientRect();
        // this gives -> width,height,left,right,top,bottom -> basically the cards position on the screen
        setPosition({
            x:e.clientX-bounds.left,
            y:e.clientY-bounds.top
        })
        // e.clientX → mouse X position in viewport
        // bounds.left → left edge of the card
//         Subtracting gives position relative to card
// So now your glow follows the mouse.
    }
  ...
  return (
    ...
    {/* the style prop -> the blur circle moves with the mouse */}
